...
def bootstrap_target_repo(task: str) -> Path:
    """Ensure `state/v2-phase-1/targets/<task>/` exists, is a git repo,
    has at least one commit. Idempotent — running twice is a no-op past
    the first call.

    The orchestrator's validate_or_reject rule 3 requires the path to
    exist and be a git repo. Layer 2 git-diff in coder.execute_step
    requires `git diff --name-only HEAD` to work — i.e. a HEAD commit
    must exist.
    """
    ...
    # Ensure at least one commit so HEAD resolves.
    has_head = subprocess.run(
        ["git", "rev-parse", "--verify", "-q", "HEAD"],
        cwd=str(p), capture_output=True,
    ).returncode == 0
    if not has_head:
        baseline = p / "README.md"
        if not baseline.is_file():
            baseline.write_text(
                f"# {task} calibration target\n"
                f"Generated by tools/calibration_runner.py "
